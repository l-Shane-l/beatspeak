...
size_t
writeCallback(char *data, size_t size, size_t nmemb, void *userdata) {
...
makeGetRequest() {
    ...
    if (curl) {
        curl_easy_setopt(curl, CURLOPT_URL,
                         "http://192.168.0.118:8080/heartrate");
        curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, writeCallback);
        curl_easy_setopt(curl, CURLOPT_WRITEDATA, &response);
        curl_easy_setopt(curl, CURLOPT_TIMEOUT,
                         5L);   // Set timeout to 5 seconds
...
main(int argc, char *argv[]) {
    ...
    unique_ptr<DataProcessor> processor(
        new DataProcessor(data_points_queue, &estimatedHeartRate));
    string option = argv[1];
    thread t1(getInput, ref(camInputRef));
    thread t2(trackHead, ref(camInputRef), ref(tracker),
              ref(estimatedHeartRate), ref(option));
...
void
getInput(shared_ptr<WebCam> &cam) {
...
void
trackHead(shared_ptr<WebCam> &cam, unique_ptr<HeadTracker> &tracker,
          std::atomic<int> &estimatedHeartRate, string mode) {
    ...
    while (run) {
        ...
        string nextReading =
            "Next Reading in: " + to_string(remaining_time) + " seconds";
        string hr = "Calculating Heart Rate ...";
        if (estimatedHeartRate != -1) {
            hr = "Your Estimated Heart Rate is: " +
                 to_string(estimatedHeartRate);
        }
        ...
        if (count % 100 == 0) {
            ...
            if (actualHR.empty()) {
                ...
                actualHR = "Actual Heart Rate is: " +
                           std::to_string(static_cast<int>(
                               std::round(std::stod(actualHR))));
            }
        }
        ++count;
        ...
        std::string errorStr = "";
        if (estimatedHeartRate != -1 && !actualHR.empty() &&
            actualHR != "Actual Heart Rate is: unavailable") {
        ...
        cv::rectangle(cam->getframeRGB(), rect, cv::Scalar(0, 0, 0),
                      cv::FILLED);
        cv::putText(cam->getframeRGB(), hr, cv::Point(30, 30),
                    cv::FONT_HERSHEY_COMPLEX, 0.8, cv::Scalar(255, 255, 255),
                    1);
        cv::putText(cam->getframeRGB(), actualHR, cv::Point(30, 60),
                    cv::FONT_HERSHEY_COMPLEX, 0.8, cv::Scalar(255, 255, 255),
                    1);
        cv::putText(cam->getframeRGB(), errorStr, cv::Point(30, 90),
                    cv::FONT_HERSHEY_COMPLEX, 0.8, cv::Scalar(255, 255, 255),
                    1);
        cv::putText(cam->getframeRGB(), nextReading, cv::Point(30, 120),
                    cv::FONT_HERSHEY_COMPLEX, 0.8, cv::Scalar(255, 255, 255),
                    1);
        if (mode == "movesense") {
            ...
            cv::putText(cam->getframeRGB(), wearable, cv::Point(30, 150),
...
void
processData(unique_ptr<DataProcessor> &processor) {
...
void
sendOutput(shared_ptr<WebCam> &cam) {
