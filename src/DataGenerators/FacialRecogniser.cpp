...
void FacialRecogniser::detectAndDraw() {
  if (faceDetectCooldown == false) {
    ...
    if (Data->faces.size() != 0) {
      ...
      eyes.detectMultiScale(faceArea, Data->eyes, 1.1, 3,
                            0 | CASCADE_SCALE_IMAGE, Size(3, 3));

      for (int i = 0; i < Data->eyes.size(); i++) {
        Point tmp{0, 0};
        Data->eyeCenter.push_back(tmp);
        Data->eyeCenter[i].x = cvRound(
            (Data->face.x + Data->eyes[i].x + Data->eyes[i].width * 0.5));
    ...
    if (faceDetectCooldownTime == 0) {
      faceDetectCooldownTime = 10;
      faceDetectCooldown = false;
    } else {
      faceDetectCooldownTime--;
    }
  }
}
