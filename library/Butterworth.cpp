...
int butterworth() {
  ...
  double FrequencyBands[2] = {
      1.5 / fps * 2,
      2.5 / fps * 2}; // these values are as a ratio of f/fs, where fs is
                      // sampling rate, and f is cutoff frequency
...
// vector<double> filter(int ord, vector<double> a, vector<double> b,
// vector<double> x)
//{
//	int np = x.size();
//	vector<double> y(np);
...
// vector<double> bandpass(vector<double> input, double lowpass, double
// highpass, double fps)
//{
//	double N = input.size();
