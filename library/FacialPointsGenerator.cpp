...
void FacialPointsGenerator::generateMask(){
    ...
    fillConvexPoly(maskedFrame, faceRegion, (255));
     for(int i =0; i < Data->eyes.size(); i++){
            ellipse2Poly(Data->eyeCenter[i], (Size((Data->radius.width)*3,Data->eyeRadius[i]))/2, 0, 0, 360, 10, eyeRegion);
            fillConvexPoly(maskedFrame, eyeRegion, (0));
    }
}

void FacialPointsGenerator::generateTrackingPoints(){
    goodFeaturesToTrack( Data->frameGray, generatedPoints, numOfPoints, 0.001, 10, maskedFrame, 3, false, 0.04 );
...
void FacialPointsGenerator::updateTrackedPoints(shared_ptr<Raw_Data_List> &rawdata){
   if(Data->frameGray.rows == Data->oldFrame.rows && Data->frameGray.cols == Data->oldFrame.cols){
    ...
    try {
        calcOpticalFlowPyrLK(Data->oldFrame, Data->frameGray, prevPoints, currentPoints, trackingStatus, errors, Size(15,15), 3, criteria);
    }
    ...
    vector<Point2f> tmp;
    vector<double> tmpY;
    for(int i=0; i < trackingStatus.size(); i++){ //putting a huge load on the cpu
...
void FacialPointsGenerator::drawPoints(){
    Scalar pointColor{(0)}, lineColor{(0)};
        ...
        line(Data->frameRGB, Point(currentPoints[i].x-5,currentPoints[i].y), Point(currentPoints[i].x+5,currentPoints[i].y), lineColor, 1);
        line(Data->frameRGB, Point(currentPoints[i].x,currentPoints[i].y-5), Point(currentPoints[i].x,currentPoints[i].y+5), lineColor, 1);
    }
}
