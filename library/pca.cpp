...
std::vector<std::vector<double>> Covarience(std::vector<std::vector<double>> features){
...
int pca() {
  ...
  std::vector<std::vector<double>> features{{1,5,1,5,8},{2,5,4,3,1},{3,6,2,2,2},{4,7,3,1,2}};
  Standardization(features);
  std::vector<std::vector<double>> covMat = Covarience(features);


Eigen::MatrixXd m(covMat[0].size(),covMat.size()); // the nexted element ref is redundant because this should always be semetrical
